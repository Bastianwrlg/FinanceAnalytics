...
import { ApVendorItem } from '../../../types';
import { formatRupiah } from '../../../utils/formatters';
import { Search, Filter, Building, Landmark, PackageCheck } from 'lucide-react';

interface ApVendorTableProps {
  items: ApVendorItem[];
  onUpdateItems?: (items: ApVendorItem[]) => void;
}

export const ApVendorTable: React.FC<ApVendorTableProps> = ({ items, onUpdateItems }) => {
  ...
  const handleStatusChange = (id: string, newStatus: ApVendorItem['status']) => {
      ...
      <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-4">
        <div>
          <div className="flex items-center space-x-2">
            <span className="px-2 py-0.5 text-[11px] font-extrabold uppercase tracking-wider bg-amber-100 text-amber-900 rounded-md">
              Sheet Tab: Payable
            </span>
            <span className="text-xs text-slate-400 font-mono">({filteredItems.length} Rekanan / Vendor)</span>
          </div>
          <h3 className="text-base font-bold text-slate-900 mt-1">
            Payable — Rincian Utang Usaha Vendor Bahan Baku &amp; Bea Cukai Pita Cukai REL
          </h3>
          <p className="text-xs text-slate-500">
            Jadwal kewajiban pembayaran tagihan pita cukai negara (CK-1), supplier bahan kimia PG/VG, nikotin, dan kemasan botol
          </p>
        </div>
        ...
        <div className="flex flex-wrap items-center gap-2">
          ...
          <div className="relative">
            ...
            <input
              type="text"
              placeholder="Cari vendor atau bahan..."
              value={searchTerm}
              onChange={(e) => setSearchTerm(e.target.value)}
              className="pl-8 pr-3 py-1.5 bg-slate-50 border border-slate-300 rounded-xl text-xs text-slate-800 focus:bg-white focus:outline-none focus:border-amber-500 w-48"
            />
          </div>
          ...
          <div className="flex items-center space-x-1.5 bg-slate-50 border border-slate-200 rounded-xl px-2.5 py-1 text-xs">
            ...
            >
              ...
              <option value="Pita Cukai">Pita Cukai</option>
              <option value="Bahan Baku">Bahan Baku</option>
              <option value="Kemasan">Kemasan</option>
              <option value="Logistik & Operasional">Logistik &amp; Operasional</option>
            </select>
          </div>
          ...
          <div className="flex items-center space-x-1.5 bg-slate-50 border border-slate-200 rounded-xl px-2.5 py-1 text-xs">
            ...
            >
              ...
              <option value="Cicilan Berjalan">Cicilan Berjalan</option>
          ...
          <thead>
            <tr className="bg-slate-100 text-slate-700 font-bold uppercase tracking-wider border-b border-slate-300">
              ...
              <th className="py-2.5 px-3 min-w-[240px] max-w-[280px] sticky left-0 bg-slate-100/95 z-10 border-r border-slate-200 shadow-[1px_0_0_0_#cbd5e1]">
                Nama Rekanan / Instansi
              </th>
              <th className="py-2.5 px-3 min-w-[130px] border-r border-slate-200">Kategori</th>
              <th className="py-2.5 px-2 text-center w-20 min-w-[80px] border-r border-slate-200">Terms</th>
              ...
              <th className="py-2.5 px-3 text-right font-mono font-bold w-[125px] min-w-[125px] bg-amber-50/70 text-amber-950 border-r border-slate-200">
                TOTAL BILLING
              </th>
          ...
          <tbody className="divide-y divide-slate-100">
            {filteredItems.map((item, idx) => (
              <tr key={item.id} className="hover:bg-amber-50/30 transition">
                ...
                <td className="py-2.5 px-3 min-w-[130px] text-slate-600 border-r border-slate-100">
                  <span
                ...
                <td className="py-2.5 px-2 text-center w-20 min-w-[80px] text-slate-500 font-mono text-[11px] border-r border-slate-100">
                  <span className="px-1.5 py-0.5 bg-slate-100 rounded text-slate-700 font-medium">{item.terms}</span>
                </td>
                ...
                <td className="py-2.5 px-3 text-center w-[130px] min-w-[130px] border-r border-slate-100">
                  <select
                    value={item.status}
                    onChange={(e) => handleStatusChange(item.id, e.target.value as ApVendorItem['status'])}
          ...
          <tfoot>
            <tr className="bg-slate-900 text-white font-bold border-t-2 border-slate-800">
              <td colSpan={4} className="py-3 px-3 text-right uppercase tracking-wider text-[11px] sticky left-0 bg-slate-900 z-10 border-r border-slate-700 shadow-[1px_0_0_0_#334155]">
                TOTAL PAYABLE (UTANG USAHA &amp; CUKAI):
              </td>
